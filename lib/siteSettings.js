...
/** 현재 적용된 배경 스킨 URL을 가져옵니다 (없으면 null). */
export async function fetchSkinImage() {
  if (!isSupabaseConfigured) return { ok: false, reason: "not-configured", url: null };
  const { data, error } = await supabase
    .from("site_settings")
    .select("skin_image_url")
    .eq("id", "default")
    .maybeSingle();
  if (error) return { ok: false, reason: error.message, url: null };
  return { ok: true, url: data?.skin_image_url || null };
}

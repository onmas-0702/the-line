...
export function mixVoiceWithBackground(
  ...
  for (let ch = 0; ch < numberOfChannels; ch++) {
    ...
    for (let i = 0; i < length; i++) {
      const bgSample = bgLength > 0 ? bgData[i % bgLength] * bgVolume : 0; // 배경음악은 처음부터 끝까지, 짧으면 반복 재생
      const voiceSample = i >= offsetSamples ? voiceData[i - offsetSamples] : 0; // 목소리는 offset 이후에 시작

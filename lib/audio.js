 ...
 *  trimmed if longer. The voice plays at full, un-faded volume until it
 *  ends; only AFTER the voice finishes does the output continue for an
 *  extra `fadeOutSeconds` of background-only audio that fades linearly down
 *  to silence — so the fade always happens after the message ends, never
 *  overlapping its final seconds, regardless of how many loops the
 *  background took to get there. Output length = voiceOffsetSeconds + voice
 *  length + fadeOutSeconds. This is plain constant-volume mixing otherwise
 *  (no ducking yet). */
  ...
  for (let ch = 0; ch < numberOfChannels; ch++) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (fadeSamples > 0 && i >= voiceEndSample) {
        // 목소리가 끝난 뒤부터만 서서히 줄어듭니다(목소리와 겹치지 않음).

...
/** 전체 결과물 맨 끝에서 배경음악만 자연스럽게 사라지도록 페이드아웃하는
 *  길이(초). 배경음악이 반복 재생 중이더라도, "몇 번째 반복 중인지"와
 *  상관없이 최종 결과물의 마지막 이 시간만큼만 서서히 줄어들면 되므로
 *  루프 여부는 신경 쓸 필요가 없습니다 — mixVoiceWithBackground 안에서
 *  "전체 길이 기준 마지막 N초"로 계산합니다. */
export const BG_FADE_OUT_SECONDS = 2.5;
 ...
 *  trimmed if longer. Output length = voiceOffsetSeconds + voice length.
 *  The background (only the background — the voice is left untouched) fades
 *  out linearly over the last `fadeOutSeconds` of the whole result, however
 *  many loops it took to get there. This is plain constant-volume mixing
 *  otherwise (no ducking yet). */
export function mixVoiceWithBackground(
  ...
  const length = offsetSamples + voiceBuffer.length;
  const fadeSamples = Math.max(0, Math.min(length, Math.round(fadeOutSeconds * sampleRate)));
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (fadeSamples > 0) {
        ...
        if (remaining < fadeSamples) {
          bgSample *= remaining / fadeSamples; // 전체 결과물 마지막 구간에서만 배경음악을 서서히 줄임
        }
      }
      const voiceSample = i >= offsetSamples ? voiceData[i - offsetSamples] : 0; // 목소리는 offset 이후에 시작 (페이드 없음)

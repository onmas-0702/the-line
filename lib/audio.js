...
/** How many background-music seconds start silent before the track kicks
 *  in, in the final mix and in its on-screen waveform. */
export const BG_START_DELAY_SECONDS = 3;
 ...
 *  (and start-delay) behavior mixVoiceWithBackground() applies to the
 *  actual audio, rendered as bars so the on-screen background track lines
 *  up with what plays. */
export function computeTiledWaveformPeaks(
  buffer,
  totalSeconds,
  barsPerLoop = 48,
  startDelaySeconds = BG_START_DELAY_SECONDS
) {
  ...
  const delayBars = Math.min(totalBars, Math.round((startDelaySeconds / totalSeconds) * totalBars));
  const tiled = [];
  for (let i = 0; i < totalBars; i++) {
    tiled.push(i < delayBars ? 6 : loopPeaks[(i - delayBars) % barsPerLoop]);
  }
  return tiled;
}
 ...
 *  volume (0–1), starting `startDelaySeconds` into the voice (default 3s,
 *  so the voice gets a beat alone before the music comes in). The
 *  background loops if shorter than the remaining voice, or is trimmed if
 *  longer — the result always matches the voice's duration. This is plain
 *  constant-volume mixing (no ducking yet). */
export function mixVoiceWithBackground(
  ...
  const delaySamples = Math.max(0, Math.round(startDelaySeconds * sampleRate));
  ...
  for (let ch = 0; ch < numberOfChannels; ch++) {
    ...
    for (let i = 0; i < length; i++) {
      let bgSample = 0;
      if (i >= delaySamples && bgLength > 0) {
        bgSample = bgData[(i - delaySamples) % bgLength]; // loop background to cover the remaining voice length
      }

...
// 배경음악 트랙 하나(personal 또는 official)를 화면에서 쓰는 형태로 변환합니다.
// genres/themes는 background_tracks 테이블에 추가된 태그 배열 컬럼을 그대로
// 옮겨서, TrackRow가 태그 칩을 보여주고 필터가 걸러낼 수 있게 합니다.
...
export function AppStoreProvider({ children }) {
    ...
    async function hydrate() {
      ...
      if (isSupabaseConfigured) {
        const [audioResult, trackResult, skinResult] = await Promise.all([
          fetchAudios(),
          // 'personal'만 불러옵니다 — 다른 창작자가 추가한 공식(official) 트랙이
          // "내 라이브러리" 목록에 섞여 들어오면 안 되기 때문입니다.
          fetchBackgroundTracks({ libraryType: "personal" }),
          fetchSkinImage(),
        ]);
        if (cancelled) return;
  ...
  const addPersonalTrack = useCallback((name, { genres = [], themes = [] } = {}) => {
    setPersonalTracks((prev) => [
      ...prev,
      { id: `pbg${Date.now()}`, name, duration: "-", genres, themes },
    ]);
  }, []);
